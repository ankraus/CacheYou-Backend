...
CREATE VIEW v_caches AS
    SELECT c.cache_id, c.public, c.latitude, c.longitude, c.title, c.description, c.link, u.username, u.user_id, c.created_at, i.image_id, array_agg(t.name) AS tags
    FROM caches c
...
CREATE VIEW v_caches_image_array AS 
    SELECT c.cache_id, c.public, c.latitude, c.longitude, c.title, c.description, c.link, u.username, u.user_id, c.created_at, array_agg(i.image_id) AS image_ids, array_agg(t.name) AS tags
    FROM caches c
    JOIN caches_tags ct USING (cache_id)
    JOIN users u USING (user_id)
    JOIN tags t USING (tag_id)
