...
const createCacheSchema = yup.object().shape({
    body: yup.object().shape({
            ...
            link: yup.string().url('not url format').notRequired(),
            tags: yup.array().of(yup.string().max(50, 'tag length must be < 50')).min(1, 'tag count must be >= 1').max(3, 'tag count must be <= 3')
        })
});
